        ...
        if (kitConfigs != null) {
            for (int i = 0; i < kitConfigs.length(); i++) {
                try {
                    ...
                    mCoreCallbacks.KitListener().kitConfigReceived(currentModuleID, configuration.toString());
                    if (configuration.shouldExcludeUser(user)) {
                        mCoreCallbacks.KitListener().kitExcluded(currentModuleID, "User was required to be known, but was not");
                        continue;
                    }
                    ...
                    try {
                        mCoreCallbacks.KitListener().kitExcluded(kitConfigs.getJSONObject(i).optInt("id", -1), "exception while starting. Exception: " + e.getMessage());
                    } catch (JSONException e1) {
                        mCoreCallbacks.KitListener().kitExcluded(-1, "exception while starting. Exception: " + e.getMessage());
                    }
        ...
        for (KitIntegration provider : providers.values()) {
            try {
                if (!provider.isDisabled()) {
                    ...
                    mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), true, location);
                }
            } catch (Exception e) {
        ...
        for (KitIntegration provider : providers.values()) {
            try {
                if (!provider.isDisabled()) {
                    ...
                    mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(report), url, startTime, method, length, bytesSent, bytesReceived, requestString, responseCode);
                }
            } catch (Exception e) {
        ...
        for (KitIntegration provider : providers.values()) {
            try {
                if (!provider.isDisabled(true)) {
                    ...
                    mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(messages), optOutStatus);
                }
            } catch (Exception e) {
                        ...
                        if (provider instanceof KitIntegration.CommerceListener) {
                            ...
                            if (projectedEvents != null && projectedEvents.size() > 0) {
                                ...
                                for (int i = 0; i < projectedEvents.size(); i++) {
                                    ...
                                    if (result.getMPEvent() != null) {
                                        ...
                                        mCoreCallbacks.KitListener().onKitApiCalled("logEvent()", provider.getConfiguration().getKitId(), !MPUtility.isEmpty(report), projectedEvent);
                                ...
                                mCoreCallbacks.KitListener().onKitApiCalled("logEvent()", provider.getConfiguration().getKitId(), !MPUtility.isEmpty(reporting), filteredEvent);
                            ...
                            if (events != null) {
                                for (MPEvent expandedEvent: events) {
                                    ...
                                    mCoreCallbacks.KitListener().onKitApiCalled("logEvent()", provider.getConfiguration().getKitId(), !MPUtility.isEmpty(reporting), expandedEvent);
            ...
            if (provider instanceof KitIntegration.PushListener) {
                try {
                    if (!provider.isDisabled()) {
                        ...
                        mCoreCallbacks.KitListener().onKitApiCalled("willHandlePushMessage()", provider.getConfiguration().getKitId(), willHandlePush, intent);
                        if (willHandlePush) {
                            ((KitIntegration.PushListener) provider).onPushMessageReceived(context, intent);
                            mCoreCallbacks.KitListener().onKitApiCalled("onPushMessageReceived()", provider.getConfiguration().getKitId(), null, context, intent);
            ...
            if (provider instanceof KitIntegration.PushListener) {
                try {
                    if (!provider.isDisabled()) {
                        ...
                        mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(),onPushRegistration, token, senderId);
                ...
                if (provider instanceof KitIntegration.EventListener && !provider.isDisabled() && provider.getConfiguration().shouldLogEvent(event)) {
                    ...
                    if (projectedEvents == null) {
                        ...
                        if (eventCopy.getInfo() != null
                            ...
                            mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(messages), eventCopy);
                        }
                        ...
                        for (int i = 0; i < projectedEvents.size(); i++) {
                            ...
                            mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(messages), projectedEvent);
        ...
        for (KitIntegration provider : providers.values()) {
            try {
                if (provider instanceof KitIntegration.EventListener && !provider.isDisabled()) {
                    ...
                    getReportingManager().logAll(report);
                    mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(report), breadcrumb);
                }
            } catch (Exception e) {
        ...
        for (KitIntegration provider : providers.values()) {
            try {
                if (provider instanceof KitIntegration.EventListener && !provider.isDisabled()) {
                    ...
                    getReportingManager().logAll(report);
                    mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(report), message, eventData);
                }
            } catch (Exception e) {
        ...
        for (KitIntegration provider : providers.values()) {
            try {
                if (provider instanceof KitIntegration.EventListener && !provider.isDisabled()) {
                    ...
                    getReportingManager().logAll(report);
                    mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(report), exception, message, eventData);
                }
            } catch (Exception e) {
            ...
            try {
                if (provider instanceof KitIntegration.EventListener && !provider.isDisabled() && provider.getConfiguration().shouldLogScreen(screenEvent.getEventName())) {
                    ...
                    if (projectedEvents == null) {
                        ...
                        mCoreCallbacks.KitListener().onKitApiCalled(provider.getConfiguration().getKitId(), !MPUtility.isEmpty(report), eventName, eventInfo);
                        if (report != null && report.size() > 0) {
                            for (ReportingMessage message : report) {
